...
class PostgreSQLBinaries:
    ...
    def get_binary_path(cls, binary_name: str, version: str = None) -> Path:
        ...
        if system_binary and manager._verify_system_binary_version(system_binary, version):
        ...
        raise BinaryNotFoundError(f"Binary {binary_name} not found for PostgreSQL {version}")
    ...
    def download_postgresql(self, version: str, force: bool = False) -> Path:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                print(f"Downloading PostgreSQL {version} binary for {self._get_platform_string()}...")
    ...
    def _download_binary(self, version: str, temp_dir: Path) -> Path:
        ...
        try:
            ...
            raise DownloadError(f"Failed to download PostgreSQL {version} for {platform}: {e}")
    ...
    def _extract_txz(self, txz_path: Path, extract_dir: Path) -> Path:
        ...
        raise DownloadError(f"Could not find PostgreSQL installation structure. Found directories: {found_dirs}")
    ...
    def _verify_system_binary_version(self, binary_path: str, expected_version: str) -> bool:
